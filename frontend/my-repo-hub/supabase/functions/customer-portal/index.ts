...
serve(async (req) => {
  ...
  try {
    ...
    // Make sure a Billing Portal configuration exists, or create one and use it
    let configurationId: string | undefined;
    try {
      const configs = await stripe.billingPortal.configurations.list({ limit: 1 });
      if (configs.data.length > 0) {
        configurationId = configs.data[0].id;
        logStep("Using existing portal configuration", { configurationId });
      } else {
      ...
      logStep("Failed to fetch/create portal configuration", { message: msg });
