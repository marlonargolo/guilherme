...
def handle_new_user_flow(from_number, message_body, conversa):
    ...
    if conversa.etapa == 'inicio':
        if any(palavra in lower_msg for palavra in ['cliente', '1', 'preciso de serviço']):
            ...
            return ("Ótimo! Vou te cadastrar como cliente. Qual tipo de serviço você geralmente precisa? "
                   "(Ex: pedreiro, pintor, eletricista)")
    ...
    elif conversa.etapa == 'cadastrando_cliente':
        ...
        # Create user and profile - CORREÇÃO AQUI
        novo_user = User.objects.create_user(
            username=from_number,
            password=User.objects.make_random_password(length=12)  # Corrigido aqui
        )
        ...
        cliente = Cliente.objects.create(perfil=perfil)
        
        # Try to find matching specialty
        try:
            esp_obj = Especialidade.objects.get(nome__iexact=especialidade)
            cliente.interesses.add(esp_obj)
        ...
        return ("Cadastro concluído! Agora você pode buscar por prestadores de serviço. "
                "Por exemplo, digite: 'Preciso de um pedreiro em São Paulo'")
    
    elif conversa.etapa == 'cadastrando_prestador':
        # Similar flow for service providers
        especialidade = extract_especialidade(message_body)
        if not especialidade:
            return "Não consegui identificar a especialidade. Poderia informar novamente?"
        ...
        prestador = Prestador.objects.create(perfil=perfil)
        
        try:
            esp_obj = Especialidade.objects.get(nome__iexact=especialidade)
            prestador.especialidades.add(esp_obj)
        except Especialidade.DoesNotExist:
            pass
        ...
        return ("Cadastro concluído! Agora você pode informar sua disponibilidade e regiões atendidas. "
                "Por exemplo, digite: 'Atendo São Paulo de segunda a sexta'")
